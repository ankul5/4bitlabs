...
const createCourse = async (req, res, next) => {
  try {
    const userId = req.user._id || req.user.id;
    const schoolId = req.user.schoolId?._id || req.user.schoolId || req.body.schoolId;
    const course = await Course.create({ ...req.body, teacher_id: userId, school_id: req.body.school_id || schoolId });
    cache.del(`courses_${course.schoolId?._id || 'all'}`);
...
const getHomeSummary = async (req, res, next) => {
  try {
    ...
    const upcomingQuizzes = schoolId
      ? await Quiz.find({ school_id: schoolId, status: 'published', limit: 5 })
      : [];
